...
const Parallax = ({ backgroundImage, title }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const scrolled = window.scrollY;
      setScrollEffect(scrolled * (isMobile ? 0.1 : 0.2)); // Adjust the effect for mobile
    };
  ...
  const style = {
    backgroundImage: `url(${backgroundImage})`,
    backgroundSize: "cover",
    backgroundPosition: `center ${50 + scrollEffect * 0.1}%`, // Adjust the parallax position
    height: "50vh",
    transition: "background-position 0.1s ease-out",
  };
  ...
  const styleContent = {
    ...
    backdropFilter: "blur(2px)",
    boxShadow:
      "6px 6px 15px rgba(35, 179, 204, 0.94), -6px -4px 15px rgba(45, 45, 150, 0.69)",
  };
